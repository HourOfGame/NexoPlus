    ...
    private NexoBlock parseBlock(String ns, String name, ConfigurationSection s) {
        ...
        for (Map<?, ?> dropMap : s.getMapList("drops")) {
            drops.add(NexoBlock.BlockDrop.builder()
                ...
                .minAmount((int) dropMap.getOrDefault("min_amount", 1))
                .maxAmount((int) dropMap.getOrDefault("max_amount", 1))
                .chance(Double.parseDouble(String.valueOf(dropMap.getOrDefault("chance", 1.0))))
                .build());
        }
